...
const _0 = new Uint8Array(16) // eslint-disable-line
...
function cryptoScalarmult (q, n, p) {
...
function cryptoScalarmultBase (q, n) {
...
function cryptoHashblocksHl (hh, hl, m, n) {
  const wh = new Int32Array(16); const wl = new Int32Array(16)
  let bh0; let bh1; let bh2; let bh3; let bh4; let bh5; let bh6; let bh7
  let bl0; let bl1; let bl2; let bl3; let bl4; let bl5; let bl6; let bl7
  let th; let tl; let i; let j; let h; let l; let a; let b; let c; let d
...
function cryptoDignDirect (sm, m, n, sk) {
  const h = new Uint8Array(64); const r = new Uint8Array(64) // eslint skip line
...
function cryptoDignDirectRnd (sm, m, n, sk, rnd) {
  const h = new Uint8Array(64); const r = new Uint8Array(64) // eslint skip line
  let i; let j; const x = new Float64Array(64)
  const p = [gf(), gf(), gf(), gf()]
...
function curve25519sign (sm, m, n, sk, optRnd) {
  // If optRnd is provided, sm must have n + 128,
  ...
  if (optRnd) {
    smlen = cryptoDignDirectRnd(sm, m, n, edsk, optRnd)
  } else {
    smlen = cryptoDignDirect(sm, m, n, edsk)
  }
...
function curve25519signOpen (m, sm, n, pk) {
  ...
  return cryptoSignOpen(m, sm, n, edpk)
}
...
function signMessage (secretKey, msg, optRandom) {
  checkArrayTypes(msg, secretKey)
  if (secretKey.length !== 32) throw new Error('wrong secret key length')
  if (optRandom) {
    checkArrayTypes(optRandom)
    if (optRandom.length !== 64) throw new Error('wrong random data length')
    const buf = new Uint8Array(128 + msg.length)
    curve25519sign(buf, msg, msg.length, secretKey, optRandom)
    return new Uint8Array(buf.subarray(0, 64 + msg.length))
  } else {
    const signedMsg = new Uint8Array(64 + msg.length)
    curve25519sign(signedMsg, msg, msg.length, secretKey)
    return signedMsg
  }
}
...
function openMessage (publicKey, signedMsg) {
  ...
  const mlen = curve25519signOpen(tmp, signedMsg, signedMsg.length, publicKey)
...
function sign (secretKey, msg, optRandom) {
  ...
  const buf = new Uint8Array((optRandom ? 128 : 64) + msg.length)
  curve25519sign(buf, msg, msg.length, secretKey, optRandom)
...
function verify (publicKey, msg, signature) {
  ...
  return (curve25519signOpen(m, sm, sm.length, publicKey) >= 0)
}
...
export default { generateKeyPair, sharedKey, signMessage, openMessage, sign, verify }
